...
def update_config(payload: dict[str, Any] = Body(...), db: Session = Depends(get_db)) -> SettingsResponse:
...
def create_manual_input(payload: ManualInputCreate, db: Session = Depends(get_db)) -> ApiMessage:
    db.add(ManualInput(timestamp=datetime.now(timezone.utc), key=payload.key, value=payload.value, notes=payload.notes))
    db.commit()
    run_refresh(db)
    return ApiMessage(message=f"Manual input '{payload.key}' saved.", timestamp=datetime.now(timezone.utc))
...
def import_csv(series_key: str, file: UploadFile = File(...), db: Session = Depends(get_db)) -> ApiMessage:
    config = load_effective_config(db)
    count = import_indicator_csv(db, series_key, file, config["thresholds"].get(series_key))
    run_refresh(db)
    return ApiMessage(message=f"Imported {count} rows into {series_key}.", timestamp=datetime.now(timezone.utc))
